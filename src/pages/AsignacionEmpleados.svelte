            ...
            {#each pedidosEnProceso as pedido}
                <details>
                    ...
                    <ul>
                        ...
                        <li><strong>Tiempo Total Estimado</strong> {pedido.tiempoTotalEstimado}</li>
                ...
                <tbody>
                    {#each pedidosDemorados as pedido}
                        <tr>
                            ...
                            <td>{pedido.tiempoTotalEstimado}</td>
                        </tr>
                    {/each}
                </tbody>
            </table>
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th>IDs de Pedidos</th>
                    </tr>
                </thead>
                <tbody>
                    {#each empleados as empleado}
                        {#if pedidosEnProceso.some(p => p.asignaciones.some(a => a.empleado.idEmpleado === empleado.idEmpleado))}
                            <tr>
                                <td>{empleado.idEmpleado}</td>
                                <td>{empleado.nombre}</td>
                                <td>
                                    {pedidosEnProceso.filter(p => 
                                        p.asignaciones.some(a => a.empleado.idEmpleado === empleado.idEmpleado)
                                    ).length}
                                </td>
                                <td>
                                    <ul>
                                        {#each pedidosEnProceso.filter(p => 
                                            p.asignaciones.some(a => a.empleado.idEmpleado === empleado.idEmpleado)
                                        ) as pedido}
        ...
        <div class="historial-pedidos">
            ...
            <ul>
                {#each pedidosCompletados as pedido}
                    <li>
                        Pedido #{pedido.idPedido}: completado Tiempo estimado: {pedido.tiempoTotalEstimado}
                        {#each pedido.detallesPedido as detalle}
                            <li>
                                - {detalle.cantidad} {detalle.mueble.nombreMueble} de {detalle.material.nombreMaterial}
                            </li>
                        {/each}
                        <br />
                    </li>
...
<style>
    ...
    .historial-pedidos {
        ...
        border: 1px solid #ddd; /* Opcional: bordes para separar visualmente */
